...
**Your backend is running on:** `localhost:8401` ✅
...
**Change the port from `8585` to `8401`:**
...
// AFTER (Correct):
const javaURI = "http://localhost:8401";
```
...
export const javaURI = "http://localhost:8401";  // ← Make sure it's 8401!
...
Edit `assets/js/api/config.js` and change `8585` → `8401`
...
```bash
curl http://localhost:8401/api/post/all
# Should return: []
```
...
| Service | Port | Status |
|---------|------|--------|
| **Flask Backend** | **8401** | ✅ Correct - Use this! |
...
1. **Backend is on 8401** - This is confirmed and working ✅
2. **Don't change backend** - It's correct!
3. **Change frontend config** - Update to port 8401
...
Once you update the port to `8401`:
...
```javascript
// Backend API Configuration
export const javaURI = "http://localhost:8401";
...
# Find and replace 8585 with 8401
find assets -name "*.js" -type f -exec sed -i '' 's/8585/8401/g' {} +
...
| What | Where | Change |
|------|-------|--------|
| **Backend** | Flask (running) | Port **8401** ✅ |
| **Frontend Config** | `config.js` | Change to **8401** ❌→✅ |
...
Just update that one line in your frontend config from `8585` to `8401` and everything will work!
